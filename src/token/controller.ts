...
export async function token(req: Request, res: Response) {
  try {
    ...
    const finalTerm = isFmeActive !== null ? isFmeActive : term;

    const query = `SELECT token${finalTerm} FROM STUDENTINFO WHERE ROLLNO = ?`;
    const result: any = await dbQuery(query, [rollno]);
    const sendTerm = finalTerm === 1 ? result[0].token1 : result[0].token2;
    return res.json({ token: sendTerm, status: finalTerm });
